...
connect_via_ssh(){
    local site_name=$1
    [ ! -e $PAYLOADS_DIR/ssh/rsa ] && echo "$PAYLOADS_DIR/ssh/rsa does not exist" && return 1 
    ...
    ssh -Y -o "StrictHostKeyChecking=no" -i $PAYLOADS_DIR/ssh/rsa -p $ssh_port root@$ssh_host
    return $?
}
